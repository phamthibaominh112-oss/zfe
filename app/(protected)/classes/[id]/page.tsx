...
import { assignTeacher, createSession, enrollStudent, rescheduleSession, updateClass } from "@/app/actions";
...
export default async function ClassDetailPage({ params, searchParams }: { params: Promise<{id:string}>; searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  const actions = canManage ? <div className="page-actions">
    ...
    <FormDetails title="Phân công GV"><form action={assignTeacher}><input type="hidden" name="class_id" value={id}/><FormGrid>
      <SelectField label="Giáo viên" name="teacher_id" required options={(allTeachers.data||[]).map((x:any)=>({value:x.id,label:`${x.code} · ${x.full_name}`}))}/>
      <SelectField label="Vai trò" name="role" required defaultValue="Main teacher" options={["Main teacher","Co-teacher","Assistant","Cover"].map(v=>({value:v,label:v}))}/>
      <Field label="Payroll factor" name="payroll_factor" type="number" step="0.1" defaultValue={1}/>
      <div className="form-actions"><button className="button button-primary">Phân công</button></div>
    </FormGrid></form></FormDetails>
    ...
    <FormDetails title="Tạo session"><form action={createSession}><input type="hidden" name="class_id" value={id}/><FormGrid>
      <Field label="Session number" name="session_no" type="number" required defaultValue={(sessions.data?.length || 0)+1}/><Field label="Ngày học" name="scheduled_date" type="date" required/>
      ...
      <SelectField label="Hình thức" name="mode" required defaultValue={classRow.mode} options={["Online","Offline","Hybrid"].map(v=>({value:v,label:v}))}/>
      <SelectField label="Giáo viên" name="teacher_id" options={(allTeachers.data||[]).map((x:any)=>({value:x.id,label:`${x.code} · ${x.full_name}`}))}/>
  ...
  return <>
    <PageHeader eyebrow="Class profile" title={`${classRow.code} · ${classRow.name}`} description="Class profile là nguồn chuẩn cho teacher assignment, enrollment, session, progress và academic records." actions={actions}/>
    ...
    <div className="metrics-grid">
      ...
      <article className="metric-card metric-yellow"><span>Sĩ số</span><strong>{enrollments.data?.length || 0}/{classRow.capacity}</strong><small>Active enrollment records</small></article>
    ...
    <div className="grid-2">
      <Panel title="Giáo viên phụ trách" description="Main, co-teacher, assistant hoặc cover">
        {teacherAssignments.data?.length ? <div className="alert-list">{teacherAssignments.data.map((item:any)=><div className="alert-item" key={item.id}><i/><div><strong>{item.teachers?.full_name}</strong><span>{item.teachers?.code} · Payroll factor {item.payroll_factor}</span></div><Status value={item.role}/></div>)}</div> : <Empty title="Chưa phân công giáo viên" description="Academic Manager cần assign teacher trước khi vận hành."/>}
      </Panel>
    ...
    {canSeeRoster ? <Panel className="section-gap" title="Class roster" description={profile.role === "customer_service" ? "CSKH thấy enrollment cơ bản, nhưng không được đọc attendance, điểm hoặc feedback." : "Danh sách học viên trong lớp"}>
      {enrollments.data?.length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Học viên</th><th>Entry level</th><th>Target</th><th>Start</th><th>Trạng thái</th></tr></thead><tbody>{enrollments.data.map((item:any)=><tr key={item.id}><td><strong>{item.students?.code} · {item.students?.full_name}</strong></td><td>{item.students?.entry_level || "—"}</td><td>{item.target || item.students?.target || "—"}</td><td>{formatDate(item.start_date)}</td><td><Status value={item.status}/></td></tr>)}</tbody></table></div> : <Empty title="Chưa có học viên" description="Enroll học viên phù hợp với lịch và target của lớp."/>}
    </Panel> : null}
    <Panel className="section-gap" title="Session schedule" description="Mọi thay đổi lịch cần giữ audit trail; không ghi đè lịch sử">
      {sessions.data?.length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Session</th><th>Ngày / Giờ</th><th>GV</th><th>Mode</th><th>Topic</th><th>Trạng thái</th>{canManage ? <th>Điều chỉnh</th> : null}</tr></thead><tbody>{sessions.data.map((item:any)=><tr key={item.id}><td><strong>#{item.session_no}</strong></td><td>{formatDate(item.scheduled_date)}<br/><span className="muted-text">{item.start_time?.slice(0,5)}–{item.end_time?.slice(0,5)} · {item.duration_hours}h</span></td><td>{(item.session_teachers||[]).map((x:any)=>x.teachers?.full_name).filter(Boolean).join(", ") || "Chưa phân công"}</td><td>{item.mode}</td><td>{item.topic || "—"}</td><td><Status value={item.status}/></td>{canManage ? <td><details className="inline-details"><summary className="button button-secondary">Reschedule</summary><form action={rescheduleSession} className="inline-edit-form"><input type="hidden" name="session_id" value={item.id}/><input type="hidden" name="class_id" value={id}/><Field label="Ngày mới" name="new_date" type="date" required defaultValue={item.scheduled_date}/><Field label="Giờ bắt đầu" name="new_start" type="time" required defaultValue={item.start_time?.slice(0,5)}/><Field label="Giờ kết thúc" name="new_end" type="time" required defaultValue={item.end_time?.slice(0,5)}/><TextAreaField label="Lý do thay đổi" name="reason" required/><button className="button button-primary">Xác nhận đổi lịch</button></form></details></td> : null}</tr>)}</tbody></table></div> : <Empty title="Chưa có session" description="Tạo session theo lịch cố định hoặc từng buổi."/>}
    </Panel>
