...
import { AdminBatchOverrideForm, TeacherBatchOverrideForm } from "@/components/batch-override-form";
import { OverrideRequestBatchApproval } from "@/components/override-request-batch-approval";
...
export default async function WorkforcePage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
    ...
    return <>
      ...
      <Panel className="section-gap" title="Buổi dạy hôm nay" description="Check-in mở trong vòng 60 phút trước giờ học. Check-out mở từ 10 phút trước giờ kết thúc để xác nhận hoàn thành ca dạy.">
        {todaySessions.length ? <div className="workforce-session-list">{todaySessions.map((row: any) => <article className="workforce-session-card" key={row.session_id}>
          <div className="workforce-session-main"><span>{row.start_time?.slice(0,5)}–{row.end_time?.slice(0,5)}</span><strong>{row.class_code} · Buổi {row.session_no}</strong><small>{Number(row.duration_hours || 0).toLocaleString("vi-VN")} giờ · {row.teacher_role === "Assistant" ? "TA / Co-teacher" : "Teaching"} · <Status value={row.session_status} /></small></div>
      ...
      <Panel className="section-gap" title={`Buổi đã qua · ${monthLabel(month)}`} description="Xem lại session cũ. Nếu Check-in/Check-out bị thiếu do lỗi kỹ thuật, gửi yêu cầu để Admin duyệt override.">
        <TeacherBatchOverrideForm month={month} rows={pastSessions.map((row:any)=>({session_id:row.session_id,scheduled_date:row.scheduled_date,start_time:row.start_time,end_time:row.end_time,class_code:row.class_code,session_no:row.session_no,check_in_at:row.check_in_at,check_out_at:row.check_out_at,hasPending:requestBySession.get(row.session_id)?.status==="Pending"}))}/>
  ...
  if (profile.role === "academic_manager" || profile.role === "customer_service") {
    ...
    return <>
      ...
      <Flash message={params.message} error={params.error}/>
      <form className="month-filter" method="get"><label>Tháng công<input type="month" name="month" defaultValue={month}/></label><input type="hidden" name="week" value={Number(params.week || 0)}/><button className="button button-primary">Xem</button></form>
      <div className="week-nav workforce-week-nav"><Link className="button button-secondary" href={`/workforce?month=${month}&week=${Number(params.week || 0)-1}`}>← Tuần trước</Link><strong>{formatDate(weekStart)} – {formatDate(weekEnd)}</strong><Link className="button button-secondary" href={`/workforce?month=${month}&week=${Number(params.week || 0)+1}`}>Tuần sau →</Link></div>
      ...
      <Panel className="section-gap" title="Lịch làm trong tuần" description="Hiển thị cả ca đã đăng ký trong tương lai. Chỉ nút Check-in/Check-out phụ thuộc thời gian thực tế.">
  ...
  const adminMonthNextDate=new Date(`${monthDate}T00:00:00+07:00`);adminMonthNextDate.setMonth(adminMonthNextDate.getMonth()+1);const adminMonthEnd=dateOnlyString(adminMonthNextDate);
  const [{ data: staff }, { data: schedules }, { data: liveRows }, { data: statements }, { data: teacherSessions }, { data: pendingOverrideRequests }, { data: teacherMonthSessions }] = await Promise.all([
    ...
    supabase.from("sessions").select("id,session_no,scheduled_date,start_time,end_time,status,classes(code,name),session_teachers(teacher_id,role,teachers(code,full_name),teacher_session_checkins(check_in_at,check_out_at))").gte("scheduled_date",monthDate).lt("scheduled_date",adminMonthEnd).lt("scheduled_date",today).neq("status","Cancelled").is("archived_at",null).order("scheduled_date",{ascending:false}).order("start_time",{ascending:false})
  ]);
  ...
  const missingTeacherClockRows=(teacherMonthSessions||[]).flatMap((session:any)=>(session.session_teachers||[]).map((link:any)=>{const teacher=joined(link.teachers);const log=joined(link.teacher_session_checkins);return {value:`${session.id}|${link.teacher_id}`,label:`${formatDate(session.scheduled_date)} · ${session.classes?.code} · Buổi ${session.session_no} · ${teacher?.full_name||"GV"}`,sub:`${session.start_time?.slice(0,5)}–${session.end_time?.slice(0,5)} · ${link.role==="Assistant"?"TA":"Teaching"} · IN ${log?.check_in_at?"✓":"thiếu"} · OUT ${log?.check_out_at?"✓":"thiếu"}`,missing:!log?.check_in_at||!log?.check_out_at};})).filter((row:any)=>row.missing).slice(0,80);
  ...
  return <>
    ...
    <Panel className="section-gap" title={`Yêu cầu override đang chờ · ${(pendingOverrideRequests||[]).length}`} description="GV gửi yêu cầu khi bỏ lỡ Check-in/Check-out do lỗi kỹ thuật. Admin có thể chọn nhiều hoặc Select all rồi approve một lần.">
      {(pendingOverrideRequests||[]).length ? <OverrideRequestBatchApproval rows={(pendingOverrideRequests||[]).map((req:any)=>{const t=joined(req.teachers);const ses=joined(req.sessions);const cl=joined(ses?.classes);return {id:req.id,label:`${t?.full_name||"GV"} · ${cl?.code||"Class"} · Buổi ${ses?.session_no||"—"}`,sub:`${formatDate(ses?.scheduled_date)} ${ses?.start_time?.slice(0,5)||""}–${ses?.end_time?.slice(0,5)||""} · ${req.reason}`}})}/> : null}
    ...
    <Panel className="section-gap" title={`Batch override ca thiếu chấm công · ${monthLabel(month)}`} description="Select all hoặc chọn nhiều ca. Hệ thống dùng giờ lịch làm IN/OUT mặc định và cùng một lý do.">
      <AdminBatchOverrideForm month={month} rows={missingTeacherClockRows}/>
      {!missingTeacherClockRows.length?<Empty title="Không có ca thiếu IN/OUT" description="Các ca quá khứ trong tháng đều đã đủ chấm công hoặc bị hủy."/>:null}
    </Panel>
