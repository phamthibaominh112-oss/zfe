...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  if (profile.role === "admin" || profile.role === "academic_manager") {
    ...
    return <>
      ...
      {profile.role === "admin" ? <Panel className="section-gap finance-intelligence-summary" title="Business Intelligence · Live" description="Revenue map theo từng học viên/gói học và Start–End date; Cash là giao dịch thu thực tế." action={<Link className="text-link" href="/business-intelligence">Mở dashboard →</Link>}>
        <div className="metrics-grid compact-metrics">
  ...
  if (profile.role === "teacher") {
    ...
    const [sessionQuery, hours, feedback, observations, payrollStatement, compensation, livePayroll, kpiLive, placementBookings, fullPlacementTests, learningRecommendations] = await Promise.all([
      ...
      teacher ? supabase.from("placement_tests").select("id,scheduled_start,duration_minutes,status,external_token,students(code,full_name)").eq("assigned_teacher_id",teacher.id).gte("scheduled_start",`${weekStart}T00:00:00+07:00`).lte("scheduled_start",`${weekEnd}T23:59:59+07:00`).neq("status","Cancelled").order("scheduled_start") : Promise.resolve({data:[] as any[]}),
      teacher ? supabase.from("student_learning_recommendations").select("id,student_id,category,priority,title,recommendation,evidence,status,students(code,full_name)").eq("teacher_id",teacher.id).eq("status","Open").order("created_at",{ascending:false}).limit(8) : Promise.resolve({data:[] as any[]})
    ]);
    ...
    return <>
      ...
      {fullPlacementTests.data?.length ? <Panel className="section-gap" title="Placement Full Test được phân công" description="Tách riêng khỏi Speaking assessor."><div className="compact-list">{fullPlacementTests.data.map((pt:any)=>{const st=joined(pt.students);return <div className="compact-row" key={pt.id}><div><strong>{formatDate(pt.scheduled_start)} · {st?.full_name}</strong><span>{st?.code} · {pt.duration_minutes}p · {pt.external_token}</span></div><Link className="button button-yellow button-small" href="/placement">Mở</Link></div>})}</div></Panel>:null}
      {learningRecommendations.data?.length ? <Panel className="section-gap" title="Recommendation cho học viên của bạn" description="Academic / CSKH gửi recommendation dựa trên điểm và risk signal."><div className="compact-list">{learningRecommendations.data.map((r:any)=>{const st=joined(r.students);return <div className="compact-row" key={r.id}><div><strong>{r.priority} · {r.title}</strong><span>{st?.code} · {st?.full_name} · {r.recommendation}</span></div><Link className="text-link" href={`/students/${r.student_id}/learning`}>Mở hồ sơ →</Link></div>})}</div></Panel>:null}
