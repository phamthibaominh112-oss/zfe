...
export default async function ProfilePage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const { data: teacher } = profile.role === "teacher" ? await supabase.from("teachers").select("id,code,full_name,email,phone,employment_status,specialization,hourly_rate").eq("user_id", profile.id).maybeSingle() : { data: null };
