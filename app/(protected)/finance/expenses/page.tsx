...
import { archiveExpense, createExpense, updateExpense } from "@/app/actions";
...
export default async function ExpensePage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  const [{ data: categories, error }, { data: expenses }, { data: teachers }, { data: payroll }] = await Promise.all([
    ...
    supabase.from("teacher_payroll_statements").select("id,teacher_id,completed_hours,gross_amount,teacher_status,admin_status,teachers(code,full_name)").eq("payroll_month", range.start).order("created_at")
  ]);
  ...
  return <>
    ...
    <nav className="finance-subnav"><Link href="/finance">Thu phí & tái phí</Link><Link className="finance-subnav-active" href="/finance/expenses">Chi phí</Link><Link href="/payroll">Lương giáo viên</Link><Link href="/finance/reports">Báo cáo thu chi</Link><Link href="/notifications">Thông báo</Link></nav>
    ...
    <Panel className="section-gap" title="Lương giáo viên" description="Bảng lương chỉ được ghi vào chi phí sau khi GV xác nhận và Admin duyệt." action={<Link className="text-link" href={`/payroll?month=${range.month}`}>Mở quản lý lương →</Link>}>
      {payroll?.length ? <div className="table-wrap"><table><thead><tr><th>Giáo viên</th><th>Giờ hoàn thành</th><th>Mức lương</th><th>GV xác nhận</th><th>Admin</th></tr></thead><tbody>{payroll.map((row:any)=>{const teacher=joined(row.teachers);return <tr key={row.id}><td><strong>{teacher?.full_name || "Giáo viên"}</strong><br/><span className="muted">{teacher?.code || "—"}</span></td><td>{Number(row.completed_hours||0).toLocaleString("vi-VN")} giờ</td><td><strong>{formatMoney(row.gross_amount)}</strong></td><td><Status value={row.teacher_status}/></td><td><Status value={row.admin_status}/></td></tr>})}</tbody></table></div> : <Empty title="Chưa có bảng lương tháng này" description="Bảng lương được chốt tự động cuối tháng hoặc Admin có thể tổng kết thủ công tại mục Lương giáo viên."/>}
    </Panel>
