...
export default async function FinancePage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const actions = canManage ? <div className="row-actions">
    <FormDetails title="Tạo tài khoản học phí">
      <form action={createTuitionAccount}><FormGrid>
        ...
        <SelectField label="Enrollment (không bắt buộc)" name="enrollment_id" options={enrollmentOptions} />
  ...
  return <>
    <PageHeader eyebrow="Finance & customer success" title={profile.role === "student" ? "Học phí của tôi" : "Học phí & Renewal Pipeline"} description={profile.role === "student" ? "Bạn chỉ thấy tài khoản học phí và giao dịch thuộc hồ sơ của chính mình. Dữ liệu của học viên khác bị chặn tại database." : "Theo dõi package, giao dịch, công nợ, ngày tái phí và task chăm sóc. Module này không hiển thị cho giáo viên hoặc quản lý học vụ."} actions={actions} />
    ...
    <div className="grid-2 section-gap">
      ...
      </Panel> : <Panel title="Quyền riêng tư" description="Dữ liệu được giới hạn theo user account"><div className="note-box">Student role không thể xem renewal task, công nợ hoặc giao dịch của người khác. Giáo viên và Quản lý học vụ không có route và cũng không có RLS policy đọc module tài chính.</div></Panel>}
    </div>
  </>;
}
