...
export default async function FinancePage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <>
    ...
    <Panel className="section-gap" title="Gói học phí" description={`${accounts?.length || 0} tài khoản có quyền truy cập`}>
      {accounts?.length ? <div className="table-wrap tuition-table-wrap"><table className="tuition-table"><thead><tr><th>Học viên</th><th>Gói học</th><th>Giá trị</th><th>Đã đóng</th><th>Còn lại</th><th>Tiến độ giờ</th><th>Tái phí</th><th>Trạng thái</th>{canManage ? <th>Thao tác</th> : null}</tr></thead><tbody>
